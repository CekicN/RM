...
int main(int count, char* vec[])
{
	...
	while (running)
	{
		...
		{
			std::cout << "[ERROR] Message to the server wasn't sent correctly.\Wanna drop connection?\n";
			std::string input;
		...
		if (bytesIn < 0)
		{
			std::cout << "[ERROR] Message from the server wasn't received correctly.\Wanna drop connection?\n";
			std::string input;
			std::getline(std::cin, input);
			if (input.compare("Yes") == 0)
			{
